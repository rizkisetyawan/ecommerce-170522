...
import { addCount, reduceCount } from '../../redux/sliceCart';
import { PayModal } from '../../components';

function CartItem({ onAddCart, onReduceCart }) {
  return (
    <>
      <Divider sx={{ my: 2 }} />
    ...
    <Container sx={{ my: 4 }}>
      <Grid container spacing={{ xs: 5, lg: 8 }}>
        <Grid item xs={12} lg={8}>
          ...
          {[1, 2, 3, 4, 5].map(() => (
            <CartItem onAddCart={handleAddCart} onReduceCart={handleReduceCart} />
          ))}
        </Grid>
